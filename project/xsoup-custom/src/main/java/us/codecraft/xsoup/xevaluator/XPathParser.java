...
public class XPathParser {
    ...
    private static final String[] COMBINATORS = new String[]{"..//", "../", "//", "/", "|", "following-sibling::", "self::"};

    private static final String[] ESCAPED_QUOTES = new String[]{"\\\"", "\\'"};

    private static final String[] QUOTES = new String[]{"\"", "'"};

    private static final String[] HIERARCHY_COMBINATORS = COMBINATORS;

    private static final Map<String, FunctionEvaluator> FUNCTION_MAPPING = new HashMap<String, FunctionEvaluator>();

    static {
        FUNCTION_MAPPING.put("contains", param -> {
            ...
            if (Boolean.parseBoolean(param[2])) {
                ...
                return new XEvaluators.ContainsTextIgnoreWhiteSpace(param[1]);
            }
        });
        FUNCTION_MAPPING.put("starts-with", param -> {
        ...
        FUNCTION_MAPPING.put("ends-with", param -> {
    ...
    private void combinator(String combinator) {
        ...
        XPathEvaluator tmpEval = StrUtil.isEmpty(subQuery) ? new DefaultXPathEvaluator(null, null) : parse(subQuery);
        ...
        if (newEval.getEvaluator() != null) {
            switch (combinator) {
                ...
                case "/":
                    ...
                    break;
                case "..//":
                    currentEval = new CombiningEvaluator.And(newEval.getEvaluator(), new StructuralEvaluator.PreviousSibling(currentEval));
                    break;
                case "../":
                case "following-sibling::":
                    currentEval = new CombiningEvaluator.And(newEval.getEvaluator(), new StructuralEvaluator.ImmediatePreviousSibling(currentEval));
                    break;
                case "self::":
                    currentEval = new CombiningEvaluator.And(newEval.getEvaluator(), new StructuralEvaluator.Self(newEval.getEvaluator()));
                    break;
            }
        } else {
