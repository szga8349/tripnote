...
public class HttpUtils {
	...
	private static String certPath = "conf/tripnote_apiclient_cert.p12";
	 ...
	 */
	 public static String posts(String url, Map<String, String> params) {
		...
		CloseableHttpResponse response = null;
		try {
			if(wx_ssl_context == null){
				Resource resource = new ClassPathResource(certPath);
				try {
					KeyStore keystore = KeyStore.getInstance("PKCS12");
					char[] keyPassword = httpUtils.mch_id.toCharArray(); //证书密码
			...
			SSLConnectionSocketFactory sslConnectionSocketFactory = new SSLConnectionSocketFactory(wx_ssl_context, new String[] {"TLSv1", "TLSv1.1", "TLSv1.2"}, null,
	 ...
	 * @return 请求失败返回null
	 */
	public String posts(String url, String s) {
		CloseableHttpClient httpClient = null;
		HttpPost httpPost = new HttpPost(url);
		String body = null;
		CloseableHttpResponse response = null;
		try {
			if(wx_ssl_context == null){
				Resource resource = new ClassPathResource("conf/tripnote_apiclient_cert.p12");
				try {
					KeyStore keystore = KeyStore.getInstance("PKCS12");
