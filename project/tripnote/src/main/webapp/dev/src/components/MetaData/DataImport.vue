<template>
  <section class="data-import-container">
    <div class="data-import-content">
      <h4>{{$t('content.dataset.import.drsjyxx')}}</h4>
      <section class="data-import-controler" v-if="!showResult">
        ...
        <div class="upload-controler">
          ...
          <div class="upload-tips">
            ...
            <div v-html="importContent"></div>
          </div>
        </div>
      </section>
      <section class="data-import-controler" v-else>
         <!-- @click="showResultList(0)" -->
        <div class="upload-result upload-result-success">
          {{$t('content.header.daor')}}{{$t('content.common.chengg')}}:<a herf="javascripg:;">{{success}}</a>
          {{$t('content.dataset.import.tiao')}}
        </div>
         <!-- @click="showResultList(1)" -->
        <div class="upload-result upload-result-failed">
          {{$t('content.header.daor')}}{{$t('content.common.shib')}}:<a herf="javascript:;">{{total-success}}</a>
          {{$t('content.dataset.import.tiao')}}
        </div>
      </section>
...
<script>
  ...
  export default{
    ...
    computed: {
      ...
      // showSuccessList(){
      //   const vm = this
      //   this.successList.map(val=>val.state = vm.$t('content.dataset.import.drcg'))
    ...
    methods: {
      ...
      async fetchData(page=1){
        ...
        // const res = await this.$http.get(`/datasource/getDatasource?page=${page}&rows=10`)
        const res = await this.$http.get('excelMetadataInfo/getDatasource')
      ...
      upload(){
        ...
        if(fileSize>15728640){
          ...
          this.percentage = 0
          this.uploading = true
          this.isUploadCheck = this.fileValue ? true : false
          ...
          this.$http.post('/excelMetadataInfo/import', formData, {contentType: 'Multipart/form-data'}).then(res=>{
            this.percentage = 100
            Object.assign(this.$data, res.body.message)
            if(res.body.statusCode==200 && this.success != 0){
              this.uploading = false
              if(res.body.message.existInUnknowCounr == 0){
                this.isFileCheck = 2
                this.importContent = this.$t('content.dataset.import.wjjzcg', [this.success])
              }else{
                this.isFileCheck = 1
                this.importContent = this.$t('content.dataset.import.wjjzcg1', [this.success, this.existInUnknowCounr, this.existInUnknow.join(', ')])
              }
              ...
              if(res.body.message == 'template error'){
                this.importContent = this.$t('content.dataset.import.jxsbqsc')
              }else if(res.body.message == '20012'){
                this.importContent = this.$t('content.dataset.import.cg10w')
              }else{
                if(this.success == 0){
      ...
      async confirmImport(){
        ...
        this.$http.post('/excelMetadataInfo/confirm', JSON.stringify({import_time})).then(async res=>{
          const {statusCode} = res.body
      ...
      async cancelImport(){
        ...
        let import_time = this.import_time
        let res = await this.$http.post('/excelMetadataInfo/cancel', JSON.stringify({import_time}))
        const {statusCode} = res.body
        if(statusCode == 200){
          this.fileValue = ''
          this.isUploadCheck = false
