...
<div id="searchContainer">
	...
	<div class="tabCon">
		...
		<div v-if="!loading">
			<div class="boxCommon" v-if = "searchKeyword">
						...
						<!-- Datasets -->
						 	 <li class="wrapper searchItem"
						  		...
						  		<ul class="list-inline searchItemTit">
						  		 	<li class="main">
						  				<span class="tableDisc"></span>
						  		 		<span class="name"
						  		 	   v-html="emphasizeName(item.name,searchKeyword)">
						  		 	   </span>
						  		 	</li>
						  		  <li class="sub">{{$t('content.dataset.businessDetail.biem')}} :
						  		 	  <span v-ellipsis:300="emphasizeName(item.businessName,searchKeyword)">
						  		 	  </span>
						  		 	</li>
						  		  <li class="sub">{{$t('content.dataset.businessDetail.miaos')}} :
									   <span
										   	v-ellipsis:400="emphasizeName(item.description,searchKeyword)"
										   	>
						  		 	  </span>
						  		 	</li>
								  <!-- <li class="sub">最后更新时间: 			{{formatTime(JSON.parse(item.properties).last_modify_time*1000)}}
									...
									<div class="tableUrn sub search-description">{{$t('content.dataset.datasetDetail.zid')}} :
										<span v-for="(val,index) in item.fields">
											<span v-html="emphasizeName(val.fieldName,searchKeyword)"></span>
											<span v-if="val.comment!='null'" v-html="emphasizeName(val.comment,searchKeyword)"></span>
	...
	methods: {
		...
		fetchData(page = 1){
			...
			// }else{
				...
				_url = `/api/v1/search?category=${this.searchCategory}&source=${this.setSearchParams.source}&keyword=${keyword}&topicPath=${this.setSearchParams.topicPath}&upstreamsystem=${this.setSearchParams.upstreamSystem}&sourcetable=${this.setSearchParams.sourceTable}&rows=10&page=${page}`
			// }
			this.$http.get(_url)
			.then(function(res) {
				this.loading = false;
		...
		countWinHeight(){
			let windowH = $('body').height();
			$('#searchContainer').css('height', `${windowH}px`)
		},
		link(item,type){
