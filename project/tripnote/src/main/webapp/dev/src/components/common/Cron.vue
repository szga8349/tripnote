        ...
        computed: {
            ...
            summary(){
                ...
                if(this.ruleForm.planType == '执行一次' && this.ruleForm.onceDateTime){
                    ...
                    }else if(this.ruleForm.cycle == 'year'&&this.ruleForm.intervalDate){
                        content += `在[${this.ruleForm.intervalDate.getMonth()+1}-${this.ruleForm.intervalDate.getDate()}]开始执行`
                    }
                    // 在[ 18:14:00 ]开始执行。在[ 18:14:30 ]结束。
                    let start = this.ruleForm.dayStartTime && `${this.ruleForm.dayStartTime.getHours()}:${this.ruleForm.dayStartTime.getMinutes()}:${this.ruleForm.dayStartTime.getSeconds()}`
                    let end = this.ruleForm.dayEndTime && `${this.ruleForm.dayEndTime.getHours()}:${this.ruleForm.dayEndTime.getMinutes()}:${this.ruleForm.dayEndTime.getSeconds()}`
                    content += `在[ ${start} ]开始执行。在[ ${end} ]结束。`
                    // 开始日期是[ 2017-10-17 ]。结束日期是[ 2017-11-07 ]。
                    let startDate = this.ruleForm.validateStartDate && `${this.ruleForm.validateStartDate.getFullYear()}-${this.ruleForm.validateStartDate.getMonth()+1}-${this.ruleForm.validateStartDate.getDate()}`
                    let endDate = this.ruleForm.validateEndDate && `${this.ruleForm.validateEndDate.getFullYear()}-${this.ruleForm.validateEndDate.getMonth()+1}-${this.ruleForm.validateEndDate.getDate()}`
                    content += `开始日期是[ ${startDate} ]。结束日期是[ ${endDate} ]。`
                }
