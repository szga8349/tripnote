				...
				:close-on-click-modal="false">
					<el-form ref="commentForm" :model="commentForm" :rules="commentRoles">
						<el-form-item prop="commentType" style="margin-bottom: 10px;">
							<el-select v-model="commentForm.commentType" :placeholder="$tc('content.dataset.datasetDetail.qxzzs', 1)" style="width: 200px;">
							   <el-option
						 ...
						 <div class="wrapper">
						 	<div class="row">
						 		<div class="col-sm-12">
						 			...
						 			<h5>{{nameObj[item.type].text}}&nbsp;<small :class="[item.modified]">({{item.modified}})</small></h5>
...
<script>
	import {Message,
				resetValidators,
				testPrivilege} from 'mixins/common'
	export default{
		name:"comments",
		...
		data(){
			return {
				...
				// {
				// 	value: 'Example',
				// 	label: this.$t('content.dataset.datasetDetail.yyjl')
				// },
				// {
				// 	value: 'Scene',
				// 	label: this.$t('content.dataset.datasetDetail.sycj')
				// },
				// {
				// 	value: 'Description',
				// 	label: this.$t("content.dataset.datasetDetail.ms")
				// },
				// {
				// 	value: 'DQ Issue',
				// 	label: this.$t("content.dataset.datasetDetail.sjzlwt")
				// }
					{
						value: 'ParamsInfo',
						label: '属性信息问题'
				...
				nameObj:{
					...
					"Example": {text: '其他', icon:'icon icon_quality'},
					"Scene": {text: '其他', icon:'icon icon_quality'},
					"Description": {text: '其他', icon:'icon icon_quality'},
					"DQ Issue": {text: '其他', icon:'icon icon_quality'},

					"ParamsInfo": {text: '属性信息问题', icon:'icon icon_app'},
					"FieldsDes": {text: '字段描述问题', icon:'icon icon_usage'},
					"Other": {text: '其他', icon:'icon icon_quality'},
				},
			}
		},
		props: ['tableId'],
		...
		methods:{
			...
			delComment(id) {
	        	this.$confirm(this.$t('content.dataset.datasetDetail.qrscgzsm'), this.$t('content.common.czts'), {
	          ...
	          type: ''
	        	...
	        	const vm = this;
	        	vm.$http.delete('/datasets/'+vm.tableId+'/comments/' + id).
	        		then((res)=>{
