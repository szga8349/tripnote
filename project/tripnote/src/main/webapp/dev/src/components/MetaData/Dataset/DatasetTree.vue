<template>
  <ul>
    ...
    <!-- <li @click="()=>{return;}">
      <div class="tree-wrapper" v-for="(item, index) in model">
        <a style="background: #eee;"
          href="javascript:;"
          class="tit load-more"
          v-if="index == 0 && item.children.length == 0">
          加载更多
        </a>
      </div>
    </li> -->
    ...
    methods: {
      ...
      clickToggle(model){
        if(model.level == 4){
          ...
          this.$http.post(`/datasets/${_datasetId}/access`).then((res)=>{
            // console.log(res)
          })
        }
        // const res = this.$http.post(`/datasets/4546518/access`)
        const vm = this
      ...
      treeToggle(model){
        ...
        const id = !model.type && model.datasetId
