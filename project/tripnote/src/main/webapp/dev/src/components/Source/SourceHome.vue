<template id="">
  <div class="source-body">
      ...
      <el-row class="row-List" type="flex" justify="space-between">
  ...
  export default{
    ...
    methods: {
      async fetchTotalPercent(){
        ...
        const res = await this.$http.get('/space/getClusterInfo')
      ...
      async fetchGeneralInfo(){
        const res = await this.$http.get(`/space/getGeneralInfo?id=${this.$parent.projectId}`);
      ...
      async fetchTotalSpaceUsageChart(){
        ...
        const res = await this.$http.get(`/space/getTotalSpaceUsageChart?num=${num}&type=${type}&projectId=${this.$parent.projectId}`);
