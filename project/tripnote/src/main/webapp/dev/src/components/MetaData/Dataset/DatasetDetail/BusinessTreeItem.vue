<template>
	<div v-if="(folder.folder && folder.hasChild) || !folder.folder">
        ...
        >
	   	<i></i>
			<span v-ellipsis:150="emphasizeName(folder.title,keyword)"></span>
	   	<!-- <span v-ellipsis:150="folder.title"></span> -->
			<span v-if="alias" v-ellipsis:150="alias"></span>
    ...
    async loadeMore(){
      ...
      if(!message.hasNextPage){
		...
		async fetchTopicChildren(){
			...
			this.treeData = [];
			const headerData = {contentType: 'application/json', dataType: 'json'}
			const res = await this.$http.post(`/datasets/treeSelect`,params,headerData)
			const {statusCode, message} = res.body
      const { records } = message
      //添加查看更多
      if(message.hasNextPage){
