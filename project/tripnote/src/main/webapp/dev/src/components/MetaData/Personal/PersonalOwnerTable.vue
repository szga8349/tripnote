  ...
  <div class="owner-table">
    ...
    <div v-if="!loadingVisible">
      <table class="table table-bordered tableList">
        ...
        <tbody v-if="!noDataVisible">
           <item v-for="(data, index) in reShowRecordList"
            :params="{
              params:{
                path:data.path,
                level:3,
          ...
          >
	        <td>
                <div v-ellipsis="data.datasourceName"></div>
            </td>
            <td>
            ...
            <td>{{data.showState}}</td>
            <!--<td>{{data.showLifeDay}}</td>-->
            <td>
                <div class="owner-operator">
                    <!--<el-tooltip effect="dark" :content="$t('content.dataset.datasetDetail.smzqsz')" placement="top">
                        <span
                            v-if="data.isPartitioned == 'Y'"
                            @click="setLifeCycle(data.id)">
                            <i class="iconfont icon-timeshengmingzhouqi"></i>
                        </span>
                    </el-tooltip> -->
                    ...
                    <el-tooltip effect="dark" :content="data.isPrivate == 0 ? $t('content.common.yc') : $t('content.common.qxyc')" placement="top">
                        <span @click="hiddenOperate({
                            isPrivate: data.isPrivate,
                            datasetId: data.id,
                            datasetName: data.datasetName})">
                            <i class="iconfont"
                                :class="{'icon-YDUI-yincangmima': data.isPrivate == 0, 'icon-xianshiheyincangicon': data.isPrivate == 1}"></i>
                        </span>
                    </el-tooltip>

                    <el-tooltip effect="dark" :content="$t('content.common.sc')" placement="top">
                        <span @click="delTable(data.id, data.datasetName)">
      ...
      <!-- <div class="maintain-pagenation"> -->
        <!-- <div class="maintain-page-total">
            共{{totalRows}}条数据
        </div> -->
    ...
    <el-dialog :title="ownerTableName" :visible.sync="dialogOwnerVisible" custom-class="changeOwner">
        ...
        <div class="ownerCon">
            ...
            <div class="txt">
                <ul class="ownerList">
                    <li v-for="item in ownerHistory" v-if="ownerHistory.length > 0">
                        <span class="ownerName">{{item.owner}}</span>
                        {{formatTime(item.date)}}
                    </li>

                    <li v-if="ownerHistory.length == 0">
                        --
                    </li>
                </ul>
            </div>
    ...
    export default{
        ...
        methods: {
            ...
            async operatePost(){
                ...
                if(statusCode==200){
                    let messageProp = this.isOperateHide ? `${content}成功` : '删除成功'
                    this.message(`${messageProp}!`, "success")
                    const params = this.isOperateHide ? {key: this.key, currentPage: this.currentPage} : {key: this.key, currentPage: 1}
            ...
            postLifeCycle(){
                ...
                this.$refs['recordForm'].validate((valid)=> {
                    if(!valid){
                        ...
                        this.$http.post(postUrl, JSON.stringify(params), {
            ...
            async fetchLifeCycleData(){
                const self = this;
                await this.$http.get(`/lifecycle/${this.datasetId}`)
                    .then((res)=>{
                        if(res.status == 200){
                            if(res.body.data.length == 0){
            ...
            async fetchData({key='', currentPage = 1}={}){
                ...
                let self = this;
                this.key = key;
                let url = `datasets/ownedDataset?keyword=${escape(key)}&rows=10&page=${currentPage}&sort=${this.sort}&order=${this.order}`;
                const res = await self.$http.get(url);
                this.loadingVisible = false;
                if(res.body.statusCode == 200){
                    Bus.$emit('loadingVisible', false)
                    Object.assign(this.$data, res.body.message);
                    this.noDataVisible = self.records.length == 0 ? true : false;
                }
            },
