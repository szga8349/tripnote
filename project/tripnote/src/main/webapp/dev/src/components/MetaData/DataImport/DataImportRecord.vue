...
export default {
	...
	methods: {
		...
		delSubmit(){
			var self = this;
			self.$http.post(`/api/v1/systemImport/${this.selectId}/delete`)
			.then(function(ret) {
		...
		dialogSubmit(formName){
			this.$refs[formName].validate((valid) => {
				...
				if(this.dialogTitle == '修改数据源'){
					...
					this.$http.post('/api/v1/systemImport/update',
						reqUpdateData,
						{
							emulateJSON: true
						}
		...
		createTask(reqData){
			this.$http.post('/api/v1/systemImport/create',
				reqData,
				{
					emulateJSON: true
				}
