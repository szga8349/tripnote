...
App.Router.map(function() {
    ...
    this.resource('metrics', function() {
        this.resource('metricspage', {
            path: '/page/:page'
        });
        this.resource('metric', {
            path: '/:id'
        });
        this.resource('metricname', {
            path: '/name/:name'
        }, function() {
            this.resource('metricnamepage', {
                path: '/page/:page'
            });
            this.resource('metricgroup', {
                path: '/:group'
            }, function() {
                this.resource('metricnamesubpage', {
                    path: '/page/:page'
                });
            });
        });
    ...
    this.resource('flows', function() {
        this.resource('flowspage', {
            path: '/page/:page'
        });
        this.resource('flowsname', {
            path: '/name/:name'
        }, function() {
            this.resource('flowssubpage', {
                path: '/page/:page'
            });
            this.resource('flow', {
                path: '/:id'
            }, function() {
                this.resource('pagedflow', {
                    path: '/page/:page'
                });
            });
        });
            ...
            if (currentState && currentState.name) {
                switch (currentState.name) {
                    ...
                    case "metricnamepage":
                        ...
                        if (previousState && previousState.name == 'metricname') {
                            params = JSON.stringify({
                                'name': previousState.context.name,
                    ...
                    case "metricnamesubpage":
                        ...
                        if (previousState && previousState.name == 'metricgroup' && parentState && parentState.name == 'metricname') {
                            params = JSON.stringify({
                    ...
                    case "pagedapplication":
                        ...
                        if (applicationState && applicationState.name == 'applicationname') {
                            params = JSON.stringify({
                                'name': applicationState.context.applicationname,
                                'page': currentState.params.page
                            });
                        } else {
                            params = JSON.stringify(currentState.params);
                    ...
                    case "pagedproject":
                        ...
                        if (projectState && projectState.name == 'project' && applicationState && applicationState.name == 'applicationname') {
                            params = JSON.stringify({
                    ...
                    case "pagedflow":
                        ...
                        if (flowState && flowState.name == 'flow' && projectState && projectState.name == 'project' && applicationState && applicationState.name == 'applicationname') {
                            params = JSON.stringify({
                                'application': applicationState.context.applicationname,
                                'project': projectState.context.project,
...
function findAndActiveMetricGroupNode(dashboard, group) {
    ...
    if (!rootNode.isLoading()) {
        ...
        for (var i in nodes) {
            if (nodes[i] && nodes[i].data && nodes[i].data.dashboard_name == dashboard && nodes[i].data.metric_group == group && nodes[i].data.level == 2) {
                window.g_skipMetricTreeActivation = true;
                nodes[i].setActive();
                scrollToTreeNode();
                return;
            ...
            for (var i = 0; i < nodes.length; i++) {
                if (level == 0) {
                    ...
                    if (nodes[i] && nodes[i].parent && nodes[i].parent.parent) {
                        ...
                        if (nodes[i].data.id == flowId && nodes[i].parent.title == project && topParent.title == application) {
                            window.g_skipFlowTreeActivation = true;
                            nodes[i].setActive();
                            scrollToTreeNode();
                            return;
