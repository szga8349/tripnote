...
var _nameList = {
    ...
    job_name: '任务名',
    job_type: '任务类型',
    job_start_time: '任务开始时间',
    job_end_time: '任务结束时间',
    exec_id: '执行ID',
    partition_type: '分区类型',
    partition_start: '分区起点',
    partition_end: '分区终点',
    source_target_type: '源/目标 类型',
}
...
var d3LineageTooltip = function(gravity) {
    ...
    if (!(g_currentData && g_currentData.nodes))
      return '';
    var preNodesSize = g_currentData.preNodesSize;
    var postNodesSize = g_currentData.postNodesSize;
    var data;
      ...
      var _sort_list;
      if(data.node_type === 'script'){
        _sort_list = ['etl_name', 'job_name', 'job_type', 'app_name', 'owner_id', 'ref_id', 'job_start_time', 'job_end_time'];
      }else if(data.tbl_name == currentData){
        if(data.storage_type === 'hdfs'){
          _sort_list = ['storage_type', 'urn', 'preNodesSize', 'postNodesSize'];
        }else{
          _sort_list = ['storage_type', 'db_name', 'tbl_name', 'preNodesSize', 'postNodesSize'];
        }
      }else if(data.storage_type === 'hdfs'){
        _sort_list = ['storage_type', 'urn'];
      }else{
        _sort_list = ['storage_type', 'db_name', 'tbl_name'];
      }
      _sort_list.forEach(function(v,i){
        // var key = data[v];
        if(v == 'preNodesSize'){
          appendRow(v, preNodesSize, t);
        }else if(v == 'postNodesSize'){
          appendRow(v, postNodesSize, t);
        }else{
          appendRow(v, data[v], t);
        }
      ...
      function appendRow(key, value, tooltip) {
        var keyrow = $("<div>").attr("class", "key").append(_nameList[key]);
