    ...
    jQuery.i18n.properties({
        ...
        callback: function() {
          ...
          _nameList = {
              // tableName: "表名",
              // abstracted_path: '表名',
              storage_type: $.i18n.prop('cclx'),
              urn: $.i18n.prop('wydz'),
              cluster: $.i18n.prop('jq'),
              job_path: $.i18n.prop('rwlj'),
              job_name: $.i18n.prop('rwmc'),
              job_type: $.i18n.prop('rwlx'),
              job_start_time: $.i18n.prop('rwkssj'),
              job_end_time: $.i18n.prop('rwjssj'),
              exec_id: $.i18n.prop('zxid'),
              partition_type: $.i18n.prop('fqlx'),
              partition_start: $.i18n.prop('fqqd'),
              partition_end: $.i18n.prop('fqzd'),
              source_target_type: $.i18n.prop('mblx'),
              db_name: $.i18n.prop('km'),
              tbl_name: $.i18n.prop('sjmc'),
              etl_name: $.i18n.prop('clgj'),
              job_type: $.i18n.prop('rwlx'),
              job_name: $.i18n.prop('rwmc'),
              app_name: $.i18n.prop('ssyy'),
              job_start_time: $.i18n.prop('kssj'),
              job_end_time: $.i18n.prop('jssj'),
              ref_id: $.i18n.prop('lcid'),
              owner_id: $.i18n.prop('cjr'),
              preNodesSize: $.i18n.prop('syb'),
              postNodesSize: $.i18n.prop('xyb'),
          }
          encn = {
            noData: $.i18n.prop('myrwsj'),
          }
          All()
        }
    });
...
function All(){
  ...
  $(function() {
      $('#lineageType').val(getQueryString('type'));

      if (getQueryString('type') == 'dataset') {
          $('#lineageID').val(getQueryString('id'));
      } else {
          $('#application').val(getQueryString('application'));
          $('#project').val(getQueryString('project'));
          $('#flow').val(getQueryString('flow'));
      }
  }); //显示title
  ...
  (function($) {
      ...
      function setupSearch() {
          ...
          $('#graphSplitter').height(450);

          $('#nodeInfoSplitter').height(250);
      ...
      function setIframeHeight(){
        ...
        $(parent.document).find('iframe').css('height', contentHeight+'px');
      }
      function refreshLineageData() {
          ...
          if (type == 'chains') {
              ...
              url = '/api/v1/lineage/dataset/' + id + '?clearCache=' + new Date().getTime();
              $(".loading").show();
              $.get(url, function(data) {
                gCount = data.data.nodes.length;
                  if (data && data.status == "ok") {
                      $(".loading").hide();
                      ...
                      if (titleObj && data.data && data.data.urn) {
                          ...
                          titleObj.text('血缘关系：' + data.data.urn);
                      }
                      renderTables(data.data);
                      g_currentData = data.data;
                      setupDagreGraph(data.data, rotation, type);
              ...
              $.get(url, function(data) {
                  if (data && data.status == "ok") {
                      ...
                      if (titleObj && data.data && data.data.flowName) {
                          titleObj.text("血缘关系: " + application + '/' + project + '/' + data.data.flowName);
                      }
                      renderTables(data.data);
                      g_currentData = data.data;
                      setupDagreGraph(data.data, rotation, type);
              ...
              if (nodes) {
                  ...
                  for (var i = 0; i < nodes.length; i++) {
                      ...
                      if (nodes[i].node_type == 'data') {
                          var _path = nodes[i].abstracted_path;
                          var _tableName = "";
                          if (_path) {
                              _tableName = _path.split('/').pop();
                          }
                          if(nodes[i]["_sort_list"].indexOf('storage_type') != -1){
                             nodes[i]["_sort_list"].splice(nodes[i]["_sort_list"].indexOf('storage_type'),1)
                             nodes[i]["_sort_list"].push('storage_type')
                          }
                          if(nodes[i]["_sort_list"].indexOf('tbl_name') != -1){
                             nodes[i]["_sort_list"].splice(nodes[i]["_sort_list"].indexOf('tbl_name'),1)
                             nodes[i]["_sort_list"].unshift('tbl_name')
                          }
                          if (!firstDataNode) {
                              if (nodes[i]["_sort_list"]) {
                                  $.each(nodes[i]["_sort_list"], function(k, v) {
                                      dataHeader += '<th>' + _nameList[v] + '</th>';
                              ...
                              for (var j = 0; j < dataHeaderNames.length; j++) {
                                  if (nodes[i][dataHeaderNames[j]]) {
                                    ...
                                    if(nodes[i]['storage_type'] == 'hdfs'){
                                      if(dataHeaderNames[j] == 'db_name'){
                                        dataBody += '<td class="wrap-all-word">' + '</td>';
                                      }else{
                                        var nodeUrn = nodes[i]['urn'];
                                        var nodeNameArr = nodeUrn.split('/');
                                        var nodeName = nodeNameArr[nodeNameArr.length-1];
                                        dataBody += '<td class="wrap-all-word"><div class="nodeName" title="'+nodeName+'">' + nodeName + '</div></td>';
                                      }
                                    }
                                      // dataBody += '<td class="wrap-all-word">' + '</td>';
                                  }
                  ...
                  if (jobBody == '<tbody><tr id="job-table-tr-0" class="result"></tr></tbody>') {
                      jobBody = '<div style="line-height: 100px; text-align: center; border-top:1px solid #ddd;">'+encn.noData+'</div>'
                  }
      ...
      function setupDagreGraph(data, rotation, type) {
          ...
          function onSearchClick(evt) {
              ...
              } else if ($target.hasClass('search-submit')) {} else if ($target.hasClass('search-clear')) {
                  $field.val('');
                  clearResults();
              }
          }
          ...
          $(document).on('keydown', '.search .search-field, .search .search-result,.btn_search', function(evt) {
              var $target = $(this),
                  $toSelect;
          ...
          $('#search-clear').click(function() {
              ...
              $('#lineagedatatable tr').removeClass('highlight')
              $('#lineagejobtable tr').removeClass('highlight')
          });

          $('.actfull-screen').click(function() {
              if ($('body').hasClass('fullscreen')) {
                  $(this).find('i').removeClass('icon_restore').addClass('icon_fullscreen');
                  $('body').removeClass('fullscreen');
                  $(parent.document).find('.iframe').removeClass('fullscreen');
                  $(parent.document).find('iframe').css('height', '900px');
              } else {
                  $(this).find('i').removeClass('icon_fullscreen').addClass('icon_restore');
                  $('body, #mainContent').addClass('fullscreen');
                  $('#canvas').width('100%');
                  $(parent.document).find('.iframe').addClass('fullscreen');
          ...
          if (type == 'job') {
              ...
              for (var i = 0; i < data.links.length; i++) {
                if (data.links[i].type == 'job') {
                  g.setEdge(
                    data.links[i].source,
                    data.links[i].target, {
                        style: "stroke: #66B2FF; stroke-width: 1.5px; stroke-dasharray: 5, 5; fill: #fff;",
            ...
            var currentDataUrn = data.urn;
            var currentDataArr = currentDataUrn.split('/');
            currentData = currentDataArr[currentDataArr.length-1];
            var currentLib = currentDataArr[currentDataArr.length - 2]
            var arrArray = [];
              for (var i = 0; i < data.nodes.length; i++) {
                  isNode = true;
                  var schema_type = '';
                  var name;
                  var shape = "rect";
                  if (data.nodes[i]['node_type'] && data.nodes[i]['node_type'] !== 'script') {
                    schema_type = 'script';
                    var urnArr = data.nodes[i]['urn'].split('/');
                    var arrLen = urnArr.length;
                    var fileName = urnArr[arrLen-1];
                    if(data.nodes[i]['storage_type'] == 'hive'){
                      name = data.nodes[i]['tbl_name'];
                    }else if(data.nodes[i]['storage_type'] == 'hdfs'){
                      name = fileName;
                    }else if(data.nodes[i].node_type == 'data') {
                    ...
                    } else if(data.nodes[i]['job_type']) {
                      var _type = data.nodes[i]["job_type"];
                      if (_type == 'hive') {
                          _type = 'l_hive'
                      } else if (_type === 'pig') {
                          _type = 'l_pig'
                  ...
                  } else if(data.nodes[i]['node_type']=='script'){
                    schema_type = "sql";
                    name = data.nodes[i]['job_type'];
                  }else{
                    schema_type = "sql";
                  ...
                  if(data.nodes[i]['tbl_name'] === currentData &&
                    data.nodes[i]['db_name'] === currentLib){
                    isNode = true;
                    g.setNode(data.nodes[i].id, {
                        label: name,
                        style: 'fill:#277dea;rx:10;ry:10',
                        id: data.nodes[i].id,
                        shape: shape,
                        currentData: 'Y'
                    });
                  ...
                  }else if (schema_type.toLowerCase() == 'pig'){
                    isNode = true;
                    g.setNode(data.nodes[i].id, {
                        label: name,
                  ...
                  }else if (schema_type.toLowerCase() == 'script'){
                    isNode = true;
                    g.setNode(data.nodes[i].id, {
                        label: name,
                        style: 'fill:#3fb577;rx:10;ry:10',
                        id: data.nodes[i].id,
                        shape: shape
                    });
                  }else if (schema_type.toLowerCase() == 'l_pig'){
                    isNode = true;
                    g.setNode(data.nodes[i].id, {
                        label: name,
                        style: 'fill:#FFE5BC;',
                  ...
                  }else if (schema_type.toLowerCase() === 'transfer') {
                    isNode = true;
                    g.setNode(data.nodes[i].id, {
                        label: '',
                        style: 'fill:#c1c3cc;',
              ...
              for (var i = 0; i < data.links.length; i++) {
                  ...
                  if (data.links[i].type == 'job') {
                      g.setEdge(data.links[i].source, data.links[i].target, {
                          ...
                          label: '',
                          style: "stroke: #66B2FF; stroke-width: 1.5px; stroke-dasharray: 5, 5; fill: none;",
                          lineInterpolate: 'basis'
                      });
                  } else {
                      g.setEdge(data.links[i].source, data.links[i].target, {
          ...
          //     return '<p class="description" style="border: 1px solid red;">' + description + "</p>";
          // };

          var render = new dagreD3.render();
          render.shapes().middleLine = function(parent, bbox, node) {
            var w = bbox.width,
            h = bbox.height,
            points = [
              { x:   0, y:        -h/2 },
            ...
            shapeSvg = parent.insert("polyline", ":first-child")
          ...
          // svgGroup.attr('transform', 'translate(' + (currentX-40)*(-1) +','+ (currentY -40)*(-1)+')');

          g.nodes().forEach(function(d){
            var node = g.node(d);
            node.rx = node.ry = 5;
            var label = node.label;
            svgWidth += (label.length*300);
          })
          ...
          var tooltip = d3LineageTooltip('w');
          tooltip.hide();
          ...
          var zoom = d3.behavior.zoom().on("zoom", function() {
            // d3.event.translate[0] = d3.event.translate[0] - (d3.event.translate[0] - xCenterOffset);
            // d3.event.translate[1] = d3.event.translate[1] - (d3.event.translate[1] - yCenterOffset);
            var translateX = (d3.event.translate[0] + xCenterOffset);
            var translateY = (d3.event.translate[1] + yCenterOffset);
            var newd3Translate = [translateX, translateY];
              svgGroup.attr("transform", "translate(" + newd3Translate + ")" +
          ...
          var xCenterOffset = (currentX - $('#mainContent').width()/2 + 30)*(-1);
          var yCenterOffset = (currentY - 200)*(-1)
          var svgGroup = d3.select('.output');
